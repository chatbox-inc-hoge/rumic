<?php
/**
 * Created by PhpStorm.
 * User: mkkn
 * Date: 2015/04/16
 * Time: 7:10
 */

namespace Chatbox\Rumic;

use Laravel\Lumen\Application as Lumen;
use Dotenv;
use Symfony\Component\Console\Input\ArgvInput;
use Symfony\Component\Console\Output\ConsoleOutput;

use Monolog\Logger;

use Chatbox\Rumic\Http\Controllers\ControllerInterface;
use Chatbox\Rumic\Http\Controllers\ControllerCollection;

/**
 * Class Application
 * @package Chatbox\Rumic
 *
 */
class Application extends Lumen{

    protected $logHandlers = [];

    protected $baseUri = null;

    public function loadConfiguration($config=[]){
        $methods = [
            "implements",
            "middlewares",
            "routeMiddelwares",
            "providers"
        ];

        foreach($methods as $entry => $method){
            if(is_numeric($entry)){
                $entry = $method;
                $method = "configurate".ucfirst($method);
            }
            if(isset($config[$entry])){
                $this->{$method}($config[$entry]);
            }
        }
    }

    #region core setter
    /**
     * @param $abstract
     * @param $method
     */
    public function addAvailableBindgins($abstract,$method)
    {
        $this->availableBindings[$abstract] = $method;
    }

    /**
     * @param string $basePath
     */
    public function setBasePath($basePath)
    {
        $this->basePath = $basePath;
    }

    /**
     * @param string $storagePath
     */
    public function setStoragePath($storagePath)
    {
        $this->storagePath = $storagePath;
    }

    /**
     * @param string $configPath
     */
    public function setConfigPath($configPath)
    {
        $this->configPath = $configPath;
    }

    /**
     * @param string $resourcePath
     */
    public function setResourcePath($resourcePath)
    {
        $this->resourcePath = $resourcePath;
    }

    #endregion

    # region configurate

    protected function configurateImplements(array $config){
        foreach($config as $value){
            $value = $value + [null,null,true];
            list($abstract,$concrete,$shared) = $value;
            $this->bind($abstract,$concrete,$shared);
        }
    }

    protected function configurateMiddlewares(array $config){
        $this->middleware($config);
    }

    protected function configurateRouteMiddlewares(array $config){
        $this->routeMiddleware($config);
    }

    protected function configurateProviders(array $config){
        foreach($config as $value){
            $this->register($value);//lumenのregisterは第二/第三引数使ってない。
        }
    }
    # endregion

    # region routing
    /**
     * コントローラの登録
     * ControllerInterfaceのみが登録可能(クラス名でも登録可能にするため、タイプセーフでない)
     * ControllerCollectionを突っ込んでルート定義を抽出し、
     * addRouteしていく流れ。
     *
     * @param $name
     * @param $controller
     * @throws \Exception
     */
    public function controller($name,$controller){
        is_string($controller) && ($controller = $this->make($controller));

        if($controller instanceof ControllerInterface){
            $cc = new ControllerCollection($name);
            $cc = $controller->connect($cc);
            $this->registerControllerCollection($cc);
        }else{
            throw new \Exception("controller must implements ControllerInterface");
        }
    }

    protected function registerControllerCollection(ControllerCollection $cc){
        $routes = $cc->getRouteCollection();
        foreach($routes as $route){
            $this->addRoute($route["method"],$route["path"],$route["route"]);
        }
    }

    # endregion

    public function setBaseUri($baseUri){
        $this->baseUri = $baseUri;
    }

    protected function addRoute($method, $uri, $action)
    {
        parent::addRoute($method, $this->baseUri.$uri, $action); // TODO: Change the autogenerated stub
    }
    public function getPathInfo()
    {
        $path = parent::getPathInfo();
        if($this->baseUri && (strpos($path,$this->baseUri) === 0)){
            $path = substr($path,strlen($this->baseUri));
        }
        return $path;
    }

    /**
     * デフォルトのログハンドラを上書き
     * @return void
     */
    protected function registerLogBindings()
    {
        $this->singleton('Psr\Log\LoggerInterface', function () {
            return new Logger('lumen', $this->logHandlers);
//            return new Logger('lumen', [$this->getMonologHandler()]);
        });
    }

    /**
     * Env読み込みのエイリアス
     * @param $dir
     */
    public function loadEnv($dir){
        Dotenv::load($dir);
    }

    /**
     * artisan 実行ヘルパー
     */
    public function artisan(){
        $kernel = $this->make(
            'Illuminate\Contracts\Console\Kernel'
        );
        exit($kernel->handle(new ArgvInput, new ConsoleOutput));
    }
}